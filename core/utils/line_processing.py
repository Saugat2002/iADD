"""
Line detection and processing utilities.
...
def filter_lines_by_length(lines: np.ndarray, min_length: float = 20.0) -> np.ndarray:
    """
    ...
    Parameters:
        ...
        min_length : float, optional, default=20.0
            Minimum length threshold for keeping detected line segments.
            Reduced from 30.0 to handle smaller generated images.
...
def filter_lines_by_main_directions(
    ...
    Returns the lines and their cluster labels.
    """
    if lines.shape[0] == 0 or angles.shape[0] == 0:
        return np.empty((0, 1, 4), dtype=lines.dtype), np.empty((0,), dtype=int)

    kmeans = KMeans(n_clusters=k, random_state=0, n_init="auto")
    labels = kmeans.fit_predict(angles)
    centers = kmeans.cluster_centers_

    # Keep lines close to centers
...
def detect_and_format_lines(
    ...
    Returns:
        lines_2D: (6, N) homogeneous lines
        labels: (N,) cluster indices for each line
    """
    ...
    clustered, labels = filter_lines_by_main_directions(filtered, angles, k=angle_cluster_k)
    if clustered.size == 0:
